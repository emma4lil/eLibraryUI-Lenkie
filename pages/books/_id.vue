  ...
  <v-row>
    <v-col cols="12">
      <v-card>
        <v-row>
          <v-col cols="12" lg="2">
            <v-img width="300" max-height="600" :src="book?.coverUrl" />
            <div class="ma-2 d-flex flex-column">
              <div>
                <span class="font-weigth-thin">AUTHOR: </span><span class="text-h6">{{book.author}}</span>
              </div>
              <div>
                <span class="font-weigth-thin">ISBN: </span><span class="text-h6">{{book?.isbn}}</span>
              </div>
            </div>
          </v-col>
          <v-col cols="12" lg="10">
            <v-card-title primary-title class="text-h2">
              {{book?.title}} <v-chip color="green" class="ml-5 white--text">
                Avialable
              </v-chip>
            </v-card-title>
...
export default {
  ...
  async mounted() {
    const result = await this.$getBookDetail(this.$route.params.id)
